...
//Define gravity
#define GRAVITY -9.81
__device__ void find_rotation_corotational_energy_minisation(float R[3][3], AFEM::element *in_element, AFEM::position_3D *current_pos, AFEM::position_3D *original_pos){
...
__device__ void find_Jacobian_localK_energy_minisation_corotational(AFEM::element *in_element, AFEM::position_3D *in_pos, AFEM::position_3D *original_pos){
	...
	//Initi
	float R[3][3];
	find_rotation_corotational_energy_minisation(R, in_element, in_pos, original_pos);
...
__global__ void gpu_make_K_energy_corotational(AFEM::element *in_vec, AFEM::position_3D *in_pos, AFEM::position_3D *original_pos, int numElem, int numNodes, float *K_d, float *M_d, float *f_d,float *RKx_matrix_d)
{
	...
	if (x < numElem){
		find_Jacobian_localK_energy_minisation_corotational(&in_vec[x], in_pos, original_pos);
...
__global__ void find_A_b_energy_minimisation_corotational(float *K_in, float *xInitial, float *RKx_matrix, float *f_ext, float *RHS, float *M_in, float *LHS, int num_nodes, float dt, float alpha, float rk, int dim, AFEM::stationary *stationary, int num_station, AFEM::sudo_force_struct * sudo_force_vec, int *sudo_force_ind, int number_of_sudo_force){
	...
	if (x == 0){
		...
		for (int i = 0; i <number_of_sudo_force; i++){

			int node_num = sudo_force_ind[i];
			int dof_counter = 0;
